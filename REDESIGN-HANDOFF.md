...
| `#about` | `BackgroundBento.jsx` | "Background" bento: **Software I've shipped** (Centient, Pink Raft, then a pixel pet yard in `PetYard.jsx`), Education, Research (2 IEEE papers + ICIPCN certificate image), Tools (monochrome icon grid that tints to brand color on hover), Certificates, live **Local time** in Quezon City. |
